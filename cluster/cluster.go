...
type Cluster interface {
	...
	// endpoint.
	//
	// Example: curl -X GET http://galaxy.core_greeter/ping where
	// galaxy.core_greeter is a service registered in the discovery table.
	// (N.B: since it’s a logical service, it may have different back-ends)
	...
	// Publish exposes provided Handler to the private net: each func that is
	// http.HandlerFunc is being mapped to URI and connected to the internal
	// HTTP router. If HandlerSpec implements HTTPMethodsSpec, the methods map
	// is used, otherwise all methods are accepted and it's a client
	// responsibility to use correct one.
	Publish(spec HandlerSpec) error
	// Wait blocks until all of the listed services become available or context
	...
	// services became available upon context cancellation.
	Wait(ctx context.Context, specs ...HandlerSpec) error
	...
	// http.Client.
	NewClient(spec HandlerSpec, fn ...string) Client
}
